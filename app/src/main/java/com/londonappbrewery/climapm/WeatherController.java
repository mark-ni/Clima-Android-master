...
public class WeatherController extends AppCompatActivity {
    ...
    final String APP_ID = "8e02791f41f9c0a6cfab5b405a68d6e5";
    // Time between location updates (5000 milliseconds or 5 seconds)
    final long MIN_TIME = 5000;
    // Distance between location updates (1000m or 1km)
    final float MIN_DISTANCE = 1000;
    // Code for requesting location data
    final int REQUEST_CODE = 222;
    ...
    protected void onResume(Bundle savedInstanceState) {
        ...
        Log.d("Clima", "Getting weatehr for current location");
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Log.d("Clima", "Permission was granted.");
                getWeatherForCurrentLocation();
            }
        }
    }
    ...
    private void letsDoSomeNetworking(RequestParams params) {
        ...
        client.get(WEATHER_URL, params, new JsonHttpResponseHandler() {
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                Log.d("Clima", "Successfully retrieved data.");

                updateUI(weatherData);
            }
            public void onFailure(int statusCode, Header[] headers, Throwable e, JSONObject response) {
                Log.d("Clima", "Fail " + e.toString());
                Toast.makeText(WeatherController.this, "Request for data failed. ",
    ...
    private void updateUI(WeatherDataModel weather) {
        ...
        int resourceID = getResources().getIdentifier(weather.getIconName(), "drawable",
                getPackageName());
